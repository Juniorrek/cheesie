jQuery(document).ready(function($) {
    ...
    $( document ).on( 'click', '.widget_multi_add_field', drape_widget_multi_add_field )
        .on( 'change', '.widget_multi_single_field', drape_widget_multi_single_field )
        .on( 'click', '.widget_multi_remove_field', drape_widget_multi_remove_field )

    function drape_set_multi_add_field() {
    ...
    drape_set_multi_add_field();
    
    function drape_widget_multi_add_field(e) {
        ...
        e.preventDefault();
            ...
            drape_widget_multi_write( $control );
    }

    function drape_widget_multi_single_field() {
        var $control = $( this ).parents( '.widget_multi_input' );
        drape_widget_multi_write( $control );
    }

    function drape_widget_multi_remove_field(e) {
        e.preventDefault();
        var $this = $( this );
        var $control = $this.parents( '.widget_multi_input' );
        $this.parent().remove();
    ...
    function drape_widget_multi_write( $element) {
    ...
    $(document).on("click", ".drape-accordion", function (e) {
        e.preventDefault();
        var $button = $(this);

        $button.next('.drape-panel').slideToggle();
    ...
    function drape_contentTypeChange() {
    ...
    drape_contentTypeChange();

    function drape_widget_chosen() {
        $(".drape-widget-chosen-select").chosen({
            width: "100%"
        });
    }
    drape_widget_chosen();
